 ...
 */
export function isHighContextFile(filePath) {
 ...
 * @return {number} 优先级分数。
 */
export function getFilePriorityScore(file) {
 ...
 * @return {string} 分组键。
 */
export function getGroupKey(filePath) {
 ...
 */
export function getFileRole(filePath) {
    if (filePath === 'package.json') {
        return 'config';
    }
 ...
 */
export function buildGroupSummary(files) {
    const groups = new Map();
    files.forEach((item) => {
        const key = getGroupKey(item.path);
        const current = groups.get(key) || { count: 0, roles: new Set(), total: 0, score: 0 };
        current.count += 1;
 ...
 */
export function buildFileSummary(files, limit = 12) {
 ...
 */
export function buildFilesOverview(files, limit = 12) {
    return sortFilesByPriority(files)
        .slice(0, limit)
        .map((item) => {
 ...
 */
export function buildSemanticHints(files) {
    const hints = [];
    const roles = files.reduce((current, item) => {
        const role = getFileRole(item.path);
        current[role] += 1;
        return current;
