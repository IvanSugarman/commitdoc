 ...
 * @return {string} 文件摘要。
 */
function buildFileChangeSummary(file, role, symbols, dependencyChanges, changeKinds, oldFile) {
 ...
 */
function inferChangeKinds(input) {
    const kinds = [];
    if (input.role === 'test') {
        kinds.push('test');
    }
 ...
 */
function isStructuralMove(oldFile, newFile) {
 ...
 */
function inferPrimaryIntent(changes) {
    const scores = {
