...
interface OutputProfile {
  /** 输出规模 */
  scale: OutputScale;
  /** commit summary / commit bullets 最小条数 */
  summaryMin: number;
  /** commit summary / commit bullets 最大条数 */
  summaryMax: number;
  /** description key changes 最小条数 */
  keyChangesMin: number;
  /** description key changes 最大条数 */
  keyChangesMax: number;
  /** description impact scope 最小条数 */
  impactScopeMin: number;
  /** description impact scope 最大条数 */
  impactScopeMax: number;
  /** 变更目的描述方式 */
  changePurposeStyle: string;
  /** 输出覆盖要求 */
  coverageHint: string;
}
 ...
 */
export function buildPrompt(input: PromptInput, briefType: BriefType): string {
 ...
 * @return {string} 轻量提示词。
 */
export function buildZhipuPrompt(
  input: PromptInput,
  briefType: BriefType,
): string {
  const outputProfile = buildOutputProfile(input);
 ...
 */
function buildOutputProfile(input: PromptInput): OutputProfile {
 ...
 * @return {string} 文本配置。
 */
function formatOutputProfile(profile: OutputProfile): string {
  return [
    `scale=${profile.scale}`,
 ...
 */
function buildNarrativeHint(input: PromptInput, profile: OutputProfile): string {
 ...
 */
function buildActionChecklist(input: PromptInput): string {
  const themes = getThemeChecklistItems(input);
 ...
 */
function buildReviewerFocusTemplate(input: PromptInput): string {
  const themes = getThemeChecklistItems(input);
  const semanticHints = input.semanticHints || "";
  const checks: string[] = [];
 ...
 */
function buildIROverview(input: PromptInput): string {
 ...
 */
function buildIRChanges(input: PromptInput): string {
 ...
 * @return {string} 模块簇摘要。
 */
function buildModuleClusters(input: PromptInput): string {
  const groups = new Map<string, {count: number; roles: Set<string>; total: number; files: string[]}>();

  sortChangesForPrompt(input).forEach((item) => {
    const group = getClusterKey(item.file);
    const current = groups.get(group) || {count: 0, roles: new Set<string>(), total: 0, files: []};
    current.count += 1;
 ...
 * @return {string} 证据文本。
 */
function buildPrimaryChanges(input: PromptInput): string {
  return sortChangesForPrompt(input)
    .slice(0, 8)
 ...
 */
function buildThemeChecklist(input: PromptInput): string {
 ...
 */
function buildKeyFiles(input: PromptInput): string {
  return sortChangesForPrompt(input)
    .slice(0, 10)
 ...
 */
function sortChangesForPrompt(input: PromptInput) {
 ...
 * @return {number} 优先级分数。
 */
function scoreChange(item: PromptInput["ir"]["changes"][number]) {
  const roleScoreMap = {
    script: 80,
    type: 72,
    config: 64,
  ...
  const pathScore =
    /(src\/(cli|commands|briefs|git|prompt|fallback-suggestion|model-log))/i.test(item.file) ? 70 :
 ...
 * @return {string} 模块簇键。
 */
function getClusterKey(filePath: string) {
