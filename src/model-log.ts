 ...
 */
export async function writeJsonCache(namespace: string, key: string, value: unknown): Promise<void> {
  try {
    const cachePath = getCachePath(namespace, key);
 ...
 * @return {Promise<void>} 写入完成。
 */
export async function writePipelineLog(stage: string, payload: Record<string, unknown>): Promise<void> {
  try {
