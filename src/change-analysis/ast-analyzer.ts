 ...
 */
export async function analyzeFileSemantics(filePath: string, patchContent: string): Promise<FileSemanticSnapshot> {
  ...
  try {
    ...
    const cached = await readJsonCache<FileSemanticSnapshot>('semantics', cacheKey);
 ...
 */
function collectTopLevelSymbols(sourceFile: ts.SourceFile): string[] {
 ...
 */
function extractPatchIdentifiers(patchContent: string): Set<string> {
 ...
 */
function extractDependencyChanges(patchContent: string): string[] {
